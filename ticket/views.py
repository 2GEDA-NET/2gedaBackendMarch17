...
class TicketViewSet(viewsets.ModelViewSet):
    ...
    def perform_create(self, serializer: TicketSerializer) -> None:
        ...
        )  # Assuming you have an 'event' foreign key on your User model
        ...
        if "quantity_available" in serializer.validated_data:
            ...
            if new_quantity < sold_tickets:
                raise serializers.ValidationError(
                    "Quantity available cannot be less than tickets sold."
                )
        serializer.save()
...
class GetPaymentView(APIView):
    ...
    def get(self, request, format=None):
        ...
        payment = Ticket_Payment.objects.create(
            ticket=ticket, user=request.user, amount=amount, is_initiated=True, url=url
        )
...
def buy_ticket(request):
    ...
    payment_info = request.data.get(
        "payment_info"
    )  # Payment information (e.g., payment method, amount)
    ...
    if payment_successful(payment_info):
        send_notification(
            request.user,
            f"Your ticket purchase for event '{event.title}' was successful.",
        )
        ...
        send_notification(
            request.user, f"Your ticket purchase for event '{event.title}' failed."
        )
...
def request_event_promotion(request):
    ...
    existing_request = EventPromotionRequest.objects.filter(
        user=request.user, event=event
    ).first()
    if existing_request:
        return Response(
            {"detail": "You have already requested promotion for this event"},
            status=400,
        )
...
)  # Only event organizers can access this view
def manage_event_promotion_requests(request):
    if request.method == "GET":
        ...
        if is_approved:
            send_notification(
                promotion_request.user,
                f"Your promotion request for '{promotion_request.event.title}' has been approved.",
            )
        else:
            send_notification(
                promotion_request.user,
                f"Your promotion request for '{promotion_request.event.title}' has been rejected.",
            )
...
def delete_event(request, event_id):
    ...
    if request.user == event.user:
        ...
        ticket_purchases = TicketPurchase.objects.filter(
            event=event, payment_status="Successful"
        )
...
def download_ticket_report(request, event_id):
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="ticket_report_event_{event_id}.csv"'
    )

    # Create a CSV writer
    writer = csv.writer(response)
    writer.writerow(
        ["Attendee Name", "Number of Tickets", "Purchase Date", "Price (Naira)"]
    )
...
class CreateWithdrawalRequestView(CreateAPIView):
    ...
    def perform_create(self, serializer):
        ...
        if (
            ticket_purchase.event.user == self.request.user
            and ticket_purchase.payment_status == "Successful"
        ):
            ...
            raise serializers.ValidationError(
                "You do not have permission to create this withdrawal request."
            )
    ...
    def put(self, request, *args, **kwargs):
        ...
        if is_approved:
            ...
            generate_withdrawal_history(
                withdrawal_request.user, withdrawal_request.amount
            )

            if withdrawal_success:
                return Response(
                    {"detail": "Withdrawal request approved"}, status=status.HTTP_200_OK
                )
            else:
                return Response(
            ...
            withdrawal_request.save()
            return Response(
                {"detail": "Withdrawal request rejected"}, status=status.HTTP_200_OK
            )
    ...
    def perform_create(self, serializer):
        ...
        for each_ticket in ticket:
            ...
            ticket_instance = Ticket.objects.create(
                category=category, price=price, quantity=quantity, is_free=is_free
            )
...
class GetUpcomingEvent(ListAPIView):
    ...
    def get_queryset(self):
        ...
        events = Event.objects.filter(date__gt=today) or Event.objects.filter(
...
class Paystack_Verification(APIView):
    ...
    def post(self, request, format=None):
        ...
        response = {"status": result[0], "message": result[1]}
...
class PayOnlineDone(APIView):
    ...
    def get(self, request, format=None):
        ...
        ab = json.loads(response.text)
        message = ab["message"]
        if (response.status_code == 200 and ab["status"] == True) and (
...
class WithdrawDetailView(generics.CreateAPIView):
    ...
    def post(self, request, format=None):
        ...
        withdraw = Withdraw.objects.create(
            details=payout_instance, amount=amount, is_pending=True
        )
...
class GetSalesDashboard(APIView):
    ...
    def get(self, request, format=None):
        ...
        if request.GET.get("filter") == "Weekly":
            ...
            ticket = Ticket_Payment.objects.filter(
                user=request.user, time_stamp__range=(seven_days_ago, now)
            )
            month_ticket = ticket.aggregate(Sum("total_amount"))["total_amount__sum"]
            ticket_count = ticket.aggregate(Sum("ticket_quantity"))[
                "ticket_quantity__sum"
            ]
            total_ticket_sales = all_ticket.aggregate(Sum("total_amount"))[
                "total_amount__sum"
            ]
            month_event = Event.objects.filter(
                user=request.user, date__range=(seven_days_ago, now)
            ).count()
            ...
            all_ticket = Ticket_Payment.objects.filter(user=request.user)
            ticket = Ticket_Payment.objects.filter(
                user=request.user, time_stamp__year=year
            )
            month_ticket = ticket.aggregate(Sum("total_amount"))["total_amount__sum"]
            ticket_count = ticket.aggregate(Sum("ticket_quantity"))[
                "ticket_quantity__sum"
            ...
            all_ticket = Ticket_Payment.objects.filter(user=request.user)
            ticket = Ticket_Payment.objects.filter(
                user=request.user, time_stamp__month=month
            )
            month_ticket = ticket.aggregate(Sum("total_amount"))["total_amount__sum"]
            ticket_count = ticket.aggregate(Sum("ticket_quantity"))[
                "ticket_quantity__sum"
            ...
            month_event = Event.objects.filter(
                user=request.user, date__month=month
            ).count()
            total_event = Event.objects.filter(user=request.user).count()
            current_balance = request.user.account_balance
