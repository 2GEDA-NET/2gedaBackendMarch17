...
class UserProfileUpdateAPI(viewsets.GenericViewSet):
    """User Profile Update API"""

    serializer_class = s.UserProfileUpdateSerializer

    def get_queryset(self):
        return m.UserProfile.objects.filter(user=self.request.user)

    # @swagger_auto_schema(request_body=s.UserProfileUpdateSerializer)
    @decorators.action(methods=["PUT"], detail=False, url_path="update")
    def update_profile(self, request, *args, **kwargs):
        ...
        if address:
            m.UserAddress.objects.filter(profile=request.user.profile).update(**address)
