...
class Reward(models.Model):
    user = models.ForeignKey(
        User, verbose_name=_("Each User"), on_delete=models.CASCADE
    )
    bonus = models.IntegerField(verbose_name=_("current bonus"), default=1)
    prev_bonus = models.IntegerField(
        verbose_name=_("previous bonus"), null=True, blank=True
    )
    medium = models.CharField(
        _("medium of earning"), max_length=50, choices=earning_medium, default="login"
    )
...
class Claim_Reward_Model(models.Model):
    user = models.ForeignKey(
        User, verbose_name=_("each_user"), on_delete=models.CASCADE
    )
