...
from reward.models import Claim_Reward_Model, Reward

from .serializers import Claim_Reward_Serializer, Reward_Serializers
    ...
    def get(self, request: Request, format=None) -> Response:
        ...
        try:
            if request.GET.get("medium") is not None:
                ...
                rewards = (
                    Reward.objects.filter(medium=medium, user=request.user)
                    .order_by("-time_stamp")
                    .values()
                )
...
class Get_All_Rewards(APIView):
    ...
    def get(self, request: Request, format=None) -> Response:
        counts = (
            Reward.objects.filter(medium__in=medium_list)
            .values("medium")
            .annotate(counts=Count("medium"))
        )
