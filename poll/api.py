...
class PollAPI(renderers.CrudModelRenderer, viewsets.GenericViewSet):
    ...
    def get_permissions(self):
        if self.action in ["destroy", "update", "partial_update"]:
            permissions = [IsPollCreatorPermission]
        else:
...
class AllPollsAPI(renderers.ReadOnlyModelRenderer, viewsets.GenericViewSet):
...
#             return Response(
#                 {"detail": "Poll not found or already closed."},
#                 status=status.HTTP_404_NOT_FOUND,
#             )

#         if poll.user != request.user:
#             return Response(
#                 {"detail": "You are not authorized to close this poll."},
...
#     serializer_class = (
#         AccessRequestSerializer  # Create a serializer for access requests
#     )
...
#                 user,
#                 poll.user,
#                 f"User {user.username} has requested access to your poll: {poll.question}",
#             )
...
#     serializer_class = (
#         AccessApprovalSerializer  # Create a serializer for access approval/denial
#     )
...
#                         self.request.user,
#                         user,
#                         f"Your access request for poll: {poll.question} has been approved.",
#                     )
#                 else:
...
#                         self.request.user,
#                         user,
#                         f"Your access request for poll: {poll.question} has been denied.",
#                     )
#             else:
...
#                 "You do not have permission to approve/deny access requests."
#             )


# class AccessDeclineView(generics.UpdateAPIView):
#     serializer_class = (
#         AccessApprovalSerializer  # Use the same serializer as AccessApprovalView
#     )
...
#         payment = Payment.objects.filter(user=user, status="success").latest(
#             "timestamp"
#         )
#     except Payment.DoesNotExist:
#         return Response(
#             {"detail": "You need to make a payment to cast votes."},
#             status=status.HTTP_400_BAD_REQUEST,
#         )
...
#         num_votes = item.get(
#             "num_votes", 1
#         )  # Default to 1 vote if 'num_votes' is not specified

#         try:
#             # Check if the user has been approved to vote in this poll
#             poll = Poll.objects.get(id=poll_id)
#             if user not in poll.access_requests.all():
#                 vote_notifications.append(
#                     {
#                         "detail": f"You have not been approved to vote in poll ID {poll_id}."
#                     }
#                 )
#                 continue
...
#                 vote_notifications.append(
#                     {
#                         "detail": f"This poll (ID {poll_id}) does not have a cost associated with it."
#                     }
#                 )
#                 continue
...
#                 vote_notifications.append(
#                     {
#                         "detail": f"Successfully cast {num_votes} vote(s) on poll ID {poll_id}."
#                     }
#                 )
#             else:
#                 vote_notifications.append(
#                     {
#                         "detail": f"Insufficient funds to cast {num_votes} vote(s) on poll ID {poll_id}."
#                     }
#                 )
#         except Poll.DoesNotExist:
...
#     return Response(
#         {"vote_notifications": vote_notifications}, status=status.HTTP_200_OK
#     )
...
#             poll_data.append({"poll": poll, "vote_count": vote_count})
